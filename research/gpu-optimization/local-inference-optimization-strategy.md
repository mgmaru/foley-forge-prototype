...
### 3.4 最適化の2つの方向性（実行可能性 vs 速度）

ComfyUIの最適化は、目的に応じて**2つの方向**に分けると理解しやすいです。
これは「ComfyUIは何を最適化しているのか」という根本的な問いに対する答えであり、将来FoleyForgeに最適化を追加する際の判断軸にもなります。

| 方向 | 技術 | 何を実現するか | 速度への影響 |
|------|------|--------------|-------------|
| **A. 実行可能性の確保** | メモリオフロード、量子化（fp16/bf16/fp8/NVFP4）、Dynamic VRAM、VAEタイリング | 本来VRAMに載らないモデルを「**何とか動かす**」 | **遅くなる方向**（VRAM⇔RAM転送コスト） |
| **B. 速度最適化** | SDPA選択、xformers、async offload、pinned memory、torch.compile | 同じモデルを「**より速く動かす**」 | **速くなる方向** |

#### 「圧縮」と「退避」は別物

直感的な疑問として「ComfyUIはモデルを圧縮しているのか?」がよく挙がりますが、正確には**「圧縮」と「退避」の2つの手法を組み合わせて**います。両者は併用可能で、ComfyUIは状況に応じて自動で使い分けます。

```text
量子化（圧縮）:
  モデル全体を低精度化してVRAM使用量を減らす
  fp16 → fp8 にすると重みサイズが半分になる
  → モデル自体が小さくなる

オフロード（退避）:
  モデルの一部をRAMやCPUに置き、必要な時だけVRAMに呼び出す
  → モデルは小さくならない。「使う部分だけVRAMに置く」
```

#### A方向が実現していること = 「より大きなモデルが動く」

A方向の最適化を組み合わせることで、ComfyUIは以下のような挙動を実現します。

```text
24GB必要なモデルを、

  8GB VRAMで    → 量子化 + sequential offload で動く（ただし遅い）
  12GB VRAMで   → model offload で動く（そこそこの速度）
  24GB VRAMで   → そのまま最速で動く
```

つまりComfyUIの本質は**「より大きなモデルを、より小さなVRAMで動かせるようにする」最適化**です。
ただし**速度とのトレードオフ**であり、A方向は速くなっているわけではなく、**むしろ遅くなる方向**であることに注意。B方向はその「遅くなった分」を多少取り戻したり、もともと載るモデルをさらに速くするための補完です。

#### よくある誤解の整理

| よくある認識 | 正確な理解 |
|------------|----------|
| ComfyUIはモデルを圧縮している | 半分正解。圧縮（量子化）に加え「退避（オフロード）」も併用している |
| ComfyUIは推論を速くしている | 主目的ではない。**主目的は「動かせるようにする」**ことで、A方向はむしろ遅くなる |
| より大きなパラメータのモデルを動かす最適化ではない | **逆**。「本来VRAMに載らない大きなモデルを、小さなVRAMで動かす」のが主軸 |

#### FoleyForgeへの含意

**Stable Audio Open は 1.2B / fp16 で約3GB** という軽量モデルなので、A方向の最適化が「派手に発動する場面」がそもそも少ないです。

| ユーザのVRAM | ComfyUI採用時 | FoleyForge自作時（diffusers経由） | 差 |
|------------|--------------|--------------------------------|------|
| 12GB以上 | オフロードなしで動く | オフロードなしで動く | **差なし** |
| 8〜12GB | model offload | `enable_model_cpu_offload()` 一行で同等 | **ほぼ差なし** |
| 6〜8GB | sequential offload + Dynamic VRAM | `enable_sequential_cpu_offload()` で動く（Dynamic VRAMほど洗練されていない） | 若干劣るがStable Audio Openなら影響限定的 |
| 6GB未満 | 量子化 + sequential offload | 同上（量子化は別途実装が必要） | ここだけComfyUIに分がある |

**よってComfyUI非採用の判断は、Stable Audio Openを使う限りUX的リスクが低い**。
ただし将来より大きな音声モデル（例: Stable Audio 2.0）に切り替える場合、この前提は崩れ得ます（§5.5「将来再検討する条件」参照）。
...
### 5.5 「今回」扱う最適化の境界線（スコープ宣言）

ここまで多くの最適化手法を列挙してきましたが、**実際にFoleyForgeで実装する範囲は限定的**です。
本節は「**何を実装し、何を実装しないか**」を明示的に宣言します。

> **基本方針**: Stable Audio Openが軽量モデル（fp16で約3GB）であることを前提に、
> **A方向（実行可能性）の最適化は diffusers が提供する範囲で完結させ、量子化や独自オフロードには踏み込まない**。
> **B方向（速度）の最適化も PyTorch がデフォルトで提供する範囲（SDPA）に留める**。
...
```text
   ...
   │ ─────────────── │         │ ──────────── │
   │ async offload   │         │  量子化       │
   │ pinned memory   │         │  (fp8/NVFP4) │
   │ torch.compile   │         │ 独自オフロード │
   │ 独自Attention   │         │ Dynamic VRAM │
   └─────────────────┘         └──────────────┘
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ ← スコープ境界線
   ┌─────────────────┐         ┌──────────────┐
   │  余裕があれば扱う  │         │  今回扱う      │
...
| 項目 | 方向 | 実装Phase | 規模 |
|------|------|----------|------|
| A. VRAMティア検出 + プロファイル選択 | 実行可能性 | Phase 4 | 半日〜1日 |
| B. OOMフォールバック | 実行可能性 | Phase 4 | 半日 |
| C. モデルライフサイクル管理 (FastAPI lifespan) | 共通 | Phase 4 | 半日 |
| diffusersの `enable_model_cpu_offload()` の利用 | 実行可能性 | Phase 4 | 数行 |
| diffusersの `enable_sequential_cpu_offload()` の利用 | 実行可能性 | Phase 4 | 数行 |
| PyTorch SDPAの利用（デフォルト） | 速度 | Phase 1〜 | 0行（デフォルト） |
| fp16 / bf16 への自動切替（Tierベース） | 速度 | Phase 4 | A.に含む |

#### 余裕があれば扱う（コミットしない）

| 項目 | 方向 | Phase | 備考 |
|------|------|-------|------|
| D. Best-of-N生成のバッチサイズ動的決定 | 速度 | Phase 5〜6 | ドメインロジックと密結合なので設計時に考慮 |
| E. xformersのオプショナル利用 | 速度 | Phase 6 | インストール済みユーザのみ |
| 起動時の初期診断ベンチマーク表示 | UX | Phase 6 | UXを差別化できるが必須ではない |

#### 今回扱わない（明示的に対象外）

| 項目 | 方向 | 扱わない理由 |
|------|------|-------------|
| **量子化（fp8 / NVFP4 / int4 等）** | 実行可能性 | Stable Audio Openは元から軽量。実装コスト・依存ライブラリ追加に見合わない |
| **diffusersの標準オフロードを超える独自オフロード** | 実行可能性 | Dynamic VRAM相当の実装は数千行規模。本プロジェクトのスコープを超える |
| Async offload + pinned memory | 速度 | オフロード自体が稀にしか発生しないため効果限定的 |
| torch.compile によるグラフコンパイル | 速度 | 初回コンパイル時間とのトレードオフ。Best-of-N用途には合わない |
| マルチGPU並列 | 速度 | 想定ユーザー層（個人）にとってオーバースペック |
| 独自FlashAttention統合 | 速度 | PyTorch SDPAで十分 |
| WebGPU / ONNX Runtime によるブラウザ内推論 | 配布 | 別軸の検討事項。本ドキュメントの対象外 |

#### 将来再検討する条件（トリガー）

以下の状況が発生した場合、本ドキュメントのスコープ判断を見直す。
特にA方向の最適化（量子化・独自オフロード）の再評価が必要になる可能性が高い。

- [ ] **より大きな音声モデルへの移行**（例: Stable Audio 2.0で10GB超になった場合）
  → 量子化やオフロード戦略の本格実装を検討
- [ ] **VRAM 6GB以下のユーザが主要ターゲットになった場合**
  → diffusers/bitsandbytesによる量子化の導入を検討
- [ ] **配布規模が拡大し、サポートコストが問題になった場合**
  → ハードウェア要件の厳格化 or 量子化版モデルの配布
- [ ] **生成速度がUXの主要ボトルネックになった場合**
  → torch.compile、async offload等の本格的な速度最適化を検討
- [ ] **NVFP4対応GPU（Blackwell世代）が普及した場合**
  → NVFP4量子化を検討
...
| 2026-05-28 | §3.4「最適化の2つの方向性（実行可能性 vs 速度）」と §5.5「『今回』扱う最適化の境界線」を追加。スコープを明示化 |
