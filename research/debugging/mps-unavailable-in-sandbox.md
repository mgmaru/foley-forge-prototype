...
> **用語補足：** 仮説Aの「偽装」とは何か、仮説Cで「なぜネットワーク/ファイル用の殻がGPUまで止めるのか」は **§10 補足解説（FAQ）** で詳説。
...
- **根本原因**：Claude Code のサンドボックスが **Metal/GPU アクセスを遮断**。`mps.is_available()` が殻の中だけ False になる。GPU遮断は**意図した機能ではなく default-deny の副作用**（なぜネットワーク/ファイル用の殻がGPUまで止めるのかは §10 Q1）。
...
このノートを読んで実際に出た疑問を、そのまま残す（未来の自分・他の読者用）。

### Q1. サンドボックスは「ネットワークとファイル」を絞るものでは？ なぜ GPU まで止まる？

**A. GPU遮断は“目的”ではなく、default-deny（原則すべて拒否）の“副作用”。**

- このサンドボックスで**設定できるのはネットワークとファイルだけ**（`settings.json` の `sandbox` に `network`/`filesystem` しか無い＝**GPUを切るツマミ自体が存在しない**）。
- ただしサンドボックスの原理は **「原則すべて拒否し、許可したものだけ通す」**。GPU(Metal)はOSの低レベル資源（GPUドライバ／デバイス＝IOKit 等）への到達が必要だが、それが許可リストに無いので**ついでに塞がれる**。

```
[default-deny] Metalに必要な低レベルアクセス → 許可リストに無い → 遮断（＝副作用）
```

- だから対処は「GPUを許可する」ではなく **「サンドボックスの外で動かす」**（GPUは設定項目ではないため）。
- **一般化**：Claude Code固有ではなく**隔離環境の定番の落とし穴**。Docker も `--gpus all` を付けないとGPUは見えない。「隔離環境ではGPUは既定で見えないことがある」と覚えておく。
- ※「具体的にどの低レベルアクセスが塞がれたか」は**プロファイル未確認＝推測**。経験的事実（殻ON=False／OFF=True）で行動には十分。

### Q2. 「OSが版を偽装して報告」って？ OSが嘘をつくの？

**A. 悪意ではなく、互換のために OS が“わざと低い版”を一部のプログラムへ返す仕組み。しかも今回は起きていなかった（仮説Aは棄却済み）。**

- Apple は版番号を `10.x → 11 → … → 26` と大きく変えてきた。古いアプリには「先頭が `10.` か」で判定する雑なコードがあり、`26` を見ると壊れる。
- そこで macOS は、**古いSDKでビルドされた／環境変数 `SYSTEM_VERSION_COMPAT=1` のプログラムには、本物の `26.5` でなく `10.16`（互換用の“偽の版”）を返す**。新しいプログラムには本物を返す。＝**相手によって答えを変える**。これが「偽装」の意味（報告しているのは OS 自身）。

| 相手のプログラム | OSが返すバージョン |
|---|---|
| 新しい（通常） | `26.5`（本物） |
| 古い／`SYSTEM_VERSION_COMPAT` 有効 | `10.16`（互換用の偽） |

- **身近な例**：ブラウザの **User-Agent**。すべてのブラウザが互換のため `Mozilla/5.0 …` と“歴史的な嘘”を名乗るのと同じ発想。
- **なぜ容疑者だったか**：もし効いていれば、torch は OS を `10.16`（＜14）と誤認し「MPS は macOS 14.0+ で対応」チェックに引っかかる＝症状とぴったり一致。
- **棄却の理由**：`sw_vers`＝`26.5`（本物）／`SYSTEM_VERSION_COMPAT`＝未設定／`=0` でも不変 → 偽装ではなかった。真因はサンドボックス（Q1）。

---

## 11. 付録：実際のコマンドと出力（証拠ログ）
