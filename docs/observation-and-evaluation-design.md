...
- **誤解しやすい点**：training-free でもシステムは時間とともに良くなる。ただし「モデルが学習して」ではなく、**設定②を改善する**から（§10 の外ループ）。改善は **Phase1＝開発者が手で／Phase2＝ローカルで個人ごとに自動**（§10.1）。**いずれも②だけ・①は不変＝training-free は保たれる**のが肝。
...
| 自動で賢くなるか | Phase1はならない（開発者が手で）／**Phase2でローカル自動化（②のみ）** | 設計次第で可能 |
...
| **外ループ（オフライン）** | **開発／改善のフロー**（ログを見て生成アルゴリズムを良くする） | **Phase1＝開発者が手で**／**Phase2＝ローカルで個人ごとに自動**（§10.1）。重み①は不変＝training-free |
...
### 10.1 ゲイン調整の2フェーズと「個人パーソナライズ」

外ループ（改善）は2フェーズで進める。背景の思想（人間の感性が正解／制御工学からの発想の借用）は [app-design-philosophy.md](../research/design-philosophy/app-design-philosophy.md) を参照。

| | Phase 1（現在） | Phase 2（目標） |
|---|---|---|
| 誰が②を調整 | 開発者が手で | **システムがローカルで自動** |
| 追従先 | （共通の良さ） | **その個人**（全ユーザ平均ではない＝検索のパーソナライズに近い） |
| 入力信号 | 選択・ログ | 選択・コメント |

**Phase2の仕組み（ローカル完結・サーバー不要）**：設定値②を SQLite/ファイルで保持し、調整機構が個人のログを見て書き換える。起動時 or「適用」ボタンで反映。

```mermaid
flowchart LR
    SEL["ユーザの選択・コメント"] --> TUNE["パラメータ調整機構<br/>（個人ログから②を書換）"]
    TUNE --> CFG[("個人別の設定値②<br/>SQLite / ファイル")]
    CFG -->|起動時ロード or「適用」ボタン| BE["バックエンド<br/>（その個人に最適化）"]
    BE --> GEN["次の生成"] --> SEL
    classDef on fill:#dbeafe,stroke:#2563eb,color:#000
    class SEL,TUNE,CFG,BE,GEN on
```

- **データは外に出ない** → サーバー不要・**OSS可**・プライバシー◎。
- **training-free 維持**：書き換えるのは②だけ、モデル重み①は不変。
...
| 課題 | 対策の方向 |
|---|---|
| コールドスタート（最初はデータ0） | **出荷時デフォルト**から開始、使うほど個人に寄る |
| 安定性／Goodhart（無難な音に収束） | 調整幅に上限・**探索（多様性）の下限を守る**・変更を残して戻せる |
...
- **個人パーソナライズ機構（Phase2）**：設定値②を SQLite/ファイルで保持し、調整機構が個人ログから書き換える。起動時/「適用」ボタンで反映。**ローカル完結・サーバー不要・OSS可**（§10.1）。
- **Phase2の課題**：コールドスタート（出荷時デフォルトから）／調整の安定性・Goodhart（調整幅上限・探索下限・戻せる）／個人別ゆえ他人の知見を借りない。
