...
| 軸 | 対象 | 誰が・いつ | 性質 |
|---|---|---|---|
| **軸1：グローバルゲイン** | 共通の良さ（全ユーザ共通の②） | **開発者が手で・恒久**（§10 の外ループそのもの） | 終わらない。常時回す |
...
| **Tier2（将来）** | ローカルゲインを推論し**加算的に自動**で足す（既定候補は床として常在） | 自動・推論 | 自動（加算的なので床は不変） | **FF-D008 で初期保留**。手動が頭打ちの証拠が出てから |
...
flowchart LR
    ...
    PRESET -->|「適用」ボタン| LOCAL["ローカルゲイン枠を<br/>スレートに足す（§10.3）"]
    GLOBAL["グローバルゲイン<br/>（固定・床として常在）"] --> SLATE["提示スレート"]
    LOCAL --> SLATE
    SLATE --> EAR
    classDef on fill:#dbeafe,stroke:#2563eb,color:#000
...
| **C. 嗜好の重み** | CLAP vs PQ の重み・配分の**重心**（どの戦略を厚くするか） | **可**（パーソナライズの本来の対象） | 制御アナロジーの「ゲイン」。個人の好みに追従させてよい部分 |
| **D. 既定値・利便** | デフォルトの長さ・音タイプ等 | **可**（低リスク） | 好みでも破綻でもない単なる初期値 |

**一行原則**：パーソナライズ／調整してよいのは「**好み（taste＝C/D）**」だけ。「**能力（competence＝破綻しない生成A・探索の確保B）**」はパーソナライズしない。制御の言葉では「**ゲイン（C）は個人に合わせて動かすが、操作量の上下限（A＝飽和制約）と安定余裕（B＝探索下限）は固定する**」。
...
### 10.3 パーソナライズの適用形態：加算的探索（グローバル／ローカルゲイン）

パーソナライズを導入するときの**形**。グローバルなゲインを動かす（＝全出力が変わる）のではなく、提示スレートに個人専用の枠を**足す**。②（ゲイン）を2種類に分けて捉える。

| ゲイン | 中身 | 可変性 | スレートでの役割 |
|---|---|---|---|
| **グローバルゲイン** | 開発者が調整した**共通の②**（既存3戦略：指示追従/バランス/音響美） | **不変** | 候補を生成し、スレートに**床として常に確保** |
| **ローカルゲイン** | その個人の選好に寄せた**②バイアス** | パーソナライズで調整（§10.2 の **C/D のみ**。A はハード安全域内・B は探索下限を維持） | **追加の枠**を生成 |

提示スレート ＝ `[グローバルゲインの候補（床・常在）] ＋ [ローカルゲインの候補（追加分）]`

| 性質 | 効果 |
|---|---|
| **グローバルが不変・床として常在** | **ベースライン品質は決して下がらない**（昨日良かった音に今日も到達できる）。適応する単一ゲインが無いので**発振・ドリフトしない** |
| **ローカルは置き換えず足すだけ** | パーソナライズは**単調に候補を足す**（奪わない）。外した日は個人枠を無視すればよい＝**探索（多様性）が構造的に保たれる** |

既存の3戦略を配分（例 2:2:1）で混ぜる設計の自然な拡張で、**既定戦略の枠に床を設け、「個人寄せ枠」を1つ足す**だけで実装できる。

> 「グローバル＝開発者が決める共通の良さ／ローカル＝個人の好み」と分け、**動かすのはローカルだけ**。これが「setpoint ドリフト（ゲイン適応）ではなく加算的探索」の意味。ローカルゲインを*いつ・どう*入れるか（Tier1=ユーザのプリセット適用 / Tier2=将来の自動）と、その導入段階・可否は §10.1 の Tier はしご・[FF-D008](./decisions.md) に従う（**初期は Tier0＝固定既定値のみ**）。
...
- **Tier1/2 共通の運用課題**：コールドスタート（Tier0 デフォルトから）／安定性・Goodhart（加算的探索でグローバル床を維持・調整幅上限・探索下限・戻せる）／個人別ゆえ他人の知見を借りない。
