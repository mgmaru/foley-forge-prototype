...
flowchart TD
    ...
    S3["Step3 プロンプト生成（決定論アルゴリズム）<br/>観測 ｜ 層A 整形式・必須語の残存"]:::algo
...
9. **確率性は「置き場所」を選ぶ——解釈と多様性が欲しい場所に置き、忠実さ・追跡性・再現性が欲しい場所は決定論にする。** Step2（解釈）・Step5（生成）・Best-of-N は確率的でよい（そこは*多様性*が欲しい）。一方 **Step3（構造化データ→プロンプト合成）のように忠実さ・追跡性・再現性が要る所は決定論（アルゴリズム）**にする。確率的な段をむやみに増やすと、(a) credit assignment が壊れ（原因が Step2/Step3/Step5 に分散）、(b) 再現性・実験基盤が崩れ、(c) **LLM はハルシネーション（音源の捏造・取りこぼし）面を増やす**——のに、多様性は1つも増えない（既に 3戦略＋Best-of-N＋Step5 が供給）。ただし**決定論を過信しない**：テンプレ品質が流暢な LLM 出力に劣りうるので、プロンプトビルダーは抽象化の裏に置き差し替え可能にし、LLM は開発時（オフライン）の道具として語彙・テンプレ設計に使う。根拠は [FF-D010](./decisions.md)。
...
- **Step3 が algo か LLM か → [FF-D010](./decisions.md) で決定：決定論アルゴリズム**（プロンプトビルダー＝テンプレート＋統制語彙、オンライン LLM は挟まない）。層A ゲートは決定論的修復側に確定。留保：テンプレ品質が頭打ちなら、抽象化の裏で LLM ビルダーに差し替え可（オフライン A/B の採用率で判断）。前提：Step2 スキーマが豊かな修飾語を吐くこと（§12 原則9）。
