...
flowchart LR
    ...
    PRESET -->|「適用」ボタン| LOCAL["ローカルゲインの<br/>上積みを足す（§10.3）"]
    GLOBAL["グローバルゲイン<br/>（土台・常在）"] --> SLATE["見せる候補<br/>（土台＋上積み）"]
    LOCAL --> SLATE
    SLATE --> EAR
    classDef on fill:#dbeafe,stroke:#2563eb,color:#000
...
### 10.3 パーソナライズの適用形態：土台＋上積み（加算的探索）

パーソナライズを導入するときの**形**。グローバルなゲインを動かす（＝全出力が変わる）のではなく、**固定の土台の上に個人寄せを“積む”**。

| 部分 | 中身 | 数 | 役割 |
|---|---|---|---|
| **土台** | 開発者調整の共通②で作る既定候補（3戦略：指示追従/バランス/音響美） | **F（固定・当面3）** | 常に全部出す。品質の土台 |
| **上積み** | その個人の選好に寄せた②バイアスで作る個人寄せ候補 | **K（可変・0〜M）** | 土台の上に足す追加分 |

- 総数 ＝ **F ＋ K**（Tier0 は K=0＝ちょうど F。個人寄せが付くほど増え、上限 F＋M）。
...
| 性質 | 効果 |
|---|---|
| 土台 F は常に満額・常在＋グローバルゲインを動かさない | **既定の品質は常に満点**。昨日良かった音に今日も到達でき、発振・ドリフトしない |
| 個人寄せは土台を削らず“積む”だけ | パーソナライズは**増やす方向だけ**（奪わない）。外した日は上積みを無視し土台から選べる＝**探索（多様性）が構造的に保たれる** |

**生成時間に上限がある**：時間を決めるのは表示数ではなく**生成数**（Best-of-N で裏で多めに生成。dev §5.3/§6/§7）。総生成数は **F・上積み上限 M・Best-of-N 本数**で上から抑えられ、**最悪時間が決まり突発的に増えない**。M を上げると時間が伸びるため、M は「生成時間 vs 多様性」で決める設定値（プロトタイプは小さく、実測して調整。具体値は未決）。

> 「土台＝開発者が決める共通の良さ／上積み＝個人の好み」と分け、**動かすのは上積みだけ**＝「setpoint ドリフト（ゲイン適応）ではなく加算的探索」。上積みを*いつ*入れるか（Tier1＝プリセット適用／Tier2＝将来の自動）と導入可否は §10.1 の Tier はしご・[FF-D008](./decisions.md) に従う（初期は Tier0＝土台のみ）。
...
- **上積みの上限 M と生成時間（未決）**：M を上げるほど生成数（＝生成時間）が増える。F・M・Best-of-N 本数で最悪時間は有界。M の具体値は生成時間 vs 多様性で実測調整（§10.3）。
