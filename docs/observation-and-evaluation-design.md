...
- **誤解しやすい点**：training-free でもシステムは時間とともに良くなる。ただし「モデルが学習して」ではなく、**開発者がオフラインで設定②を手で改善する**から（§10 の外ループ）。**重みは変えないまま改善する**のが肝。
...
| 自動で賢くなるか | ならない（改善は開発者が手で） | 設計次第で可能 |
...
| **外ループ（オフライン）** | **開発／改善のフロー**（ログを見て生成アルゴリズムを良くする） | **開発者が手で**（training-free なので自動の学習ループではない） |
