...
- [ ] **多角判定の具体パラメータ**（§5.4）：**DSP欠陥の閾値＝較正済み・仮値登録（2026-06-13・§5.4(3a)）**＝無音 peak<-60dBFS／クリップ率>0.1%／NaN即NG（ほぼ無音・極端SNRは記録のみ）。残り＝CLAP の扱い／**クラウド audio-LLM 選定（音声対応・モデル名/版）**／L1〜L3 の不一致処理。
