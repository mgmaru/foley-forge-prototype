...
flowchart TD
    ...
    subgraph PAR["並列・独立判定（互いの結果を見ない＝バイアス防止）"]
        ...
        L2["L2 CLAP<br/>類似スコア（記録・相対のみ）"]:::sub
        L3["L3 audio-LLM<br/>yes/部分/no"]:::sub
    end
    L0 -- "欠陥なし" --> L1
    L0 -- "欠陥なし" --> L2
    L0 -- "欠陥なし" --> L3
    L1 --> R{"L3 は L1 と一致？<br/>（L2 は記録のみ）"}
    L2 -.->|記録| LBL
    L3 --> R
...
- **③ 突き合わせ（L1 アンカー）**：**L1 の判定をそのクリップのラベルに採用**。ただし **L3 が L1 と食い違うクリップだけ人間が聴き直して L1 を確定**（見落とし・甘さの補正・**具体ルールは (3c)**）。**L2(CLAP) は合否・再聴トリガに使わず、スコアを記録して同一プロンプト内の相対比較・分析に充てる**（案B・(3b)）。L2/L3 はラベルを**上書きしない＝監査**であって否決ではない。
...
**Phase 0 での役割＝記録のみ（案B・2026-06-13 決定）**：上記に加え「**人間相関が低い**（プロジェクト既定見解）」「**verifier hacking**（決定論ゆえ抜け穴が再現性100%で突かれる→最大化しにいかない・原則8）」から、CLAP に**合否権も再聴トリガも与えない**。CLAP は**スコアを記録して同一プロンプト内の相対比較・分析に充てるだけ**。**再聴トリガ（L1 との食い違い検出）は L3 audio-LLM が担う**（L3 は yes/部分/no のラベルを返し、L1 の OK/部分/NG と直接比較できるため）。

**スコープ**：ここの CLAP は**オフラインの判定道具**。製品パイプラインの Step6 リランカー（CLAP/PAM）は **Phase 2**・範囲外（§10）。同じ CLAP でも役割が別物。

**(3c) 再聴トリガ（L1〜L3 不一致ルール・案B 下）** — どのクリップを人間が聴き直すかのルール。**ラベルを決めるのではなく、L1 単独主観の見落とし・甘さを独立した L3 で炙り出す**ための仕掛け（最終ラベルは常に L1・(2)③）。CLAP(L2) はトリガにせず記録のみ（(3b)）。
...
- [ ] **多角判定の具体パラメータ**（§5.4）：**DSP閾値＝較正済み(2026-06-13・(3a))／CLAP＝記録のみに決定(案B・(3b))／L1〜L3 不一致ルール＝確定(2026-06-13・(3c)：L3 が L1 と「到達/未到達」で食い違うクリップだけ再聴・L1 が確定・OK/部分のズレは無視)**。残り＝**クラウド audio-LLM(L3) 選定（音声対応・モデル名/版）**のみ。
