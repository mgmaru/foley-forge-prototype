...
| **品質（方向性）** | 🔴 **hard gate（低い床）** | 破綻/無音/無関係でない方向に届くか／代表プロンプト N 本（N=6・§8）中のカバー率（**判定方法は §5.4 の多角判定**） | 例：?割以上（**未確定**） |
...
> 例：3プロンプト × Best-of-N=4本 ＝ 12クリップ。A[OK,OK,部分,NG]／B[部分,NG,NG,NG]／C[NG,NG,NG,NG] → **ヒット率=5/12≈42%**（比較用）、**カバー率=2/3≈67%**（gate用）。仮に floor=50% なら 67%≥50% で **品質ゲート＝OK**。gate は甘いカバー率、優劣は厳しいヒット率。
...
純粋な直列でも並列でもない：**L0＝直列の前段ゲート → L1・L2・L3＝並列・独立 → L1基準で突き合わせ**（多数決ではない）。
...
flowchart TD
    ...
    L2 --> R
    L3 --> R
    R -- "一致" --> LBL["クリップのラベル＝L1<br/>到達=OK/部分 ／ 未到達=NG"]:::anchor
    R -- "不一致" --> RE["人間が聴き直して<br/>L1 を確定"]:::anchor
    RE --> LBL

    subgraph Legend["凡例（箱の色）"]
        direction LR
        LA["青＝人間（最終アンカー）"]:::anchor
        LS["黄＝モデルの第2/第3意見（監査）"]:::sub
        LN["赤＝自動NG（客観DSP）"]:::ng
    end
...
- **② 並列・独立判定**：欠陥なしのクリップを **L1(人間ブラインド)・L2(CLAP)・L3(LLM) が互いの結果を見ずに**判定（直列にすると L1 が他に引きずられる＝**アンカリングバイアス**で独立性が死ぬ）。※ここの **L1「人間（ブラインド）」を実装する仕組みが (4)**（どのモデルが作ったか伏せて聴く）。
- **③ 突き合わせ（L1 アンカー）**：**L1 の判定をそのクリップのラベルに採用**。ただし **L2・L3 が L1 と食い違うクリップだけ人間が聴き直して L1 を確定**（見落とし・甘さの補正）。L2/L3 はラベルを**上書きしない＝第2/第3意見の監査**であって否決ではない。
- **到達 ＝ OK または 部分**（NG は未到達）。

> **なぜ多数決でないか**：本アプリの正解は人間の感性（philosophy §1）で、**CLAP は人間相関が低い**（§5.4 表 L2）。モデルに人間と同じ投票権を与えて**上書き**させない。L2/L3 の役目は「否決」ではなく「**人間に聴き直させる引き金**」。＝「開発者1人＝主観的」を、客観DSP（自動NG）＋独立した監査で構造的に緩和する。
...
Phase 0 で手書きするプロンプトは、Stable Audio Open 等が学習した **英語の Freesound 風構造（源＋動作＋特性＋品質＋ネガティブ／FF-D010）** で書く（日本語シーンは人間用ラベル）。各シーンの英語プロンプト文は別途起案する（→ §12 未決）。
...
- **代表プロンプト集** → **§8 で確定**（source 軸・候補シーン①〜⑧）。残り：各シーンの**英語 Freesound 風プロンプト文の起案**。
