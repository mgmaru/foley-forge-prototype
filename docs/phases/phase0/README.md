...
- **③ 突き合わせ（L1 アンカー）**：**L1 の判定をそのクリップのラベルに採用**。ただし **L3 が L1 と食い違うクリップだけ人間が聴き直して L1 を確定**（見落とし・甘さの補正）。**L2(CLAP) は合否・再聴トリガに使わず、スコアを記録して同一プロンプト内の相対比較・分析に充てる**（案B・(3b)）。L2/L3 はラベルを**上書きしない＝監査**であって否決ではない。
...
- [ ] **多角判定の具体パラメータ**（§5.4）：**DSP閾値＝較正済み（2026-06-13・(3a)）／CLAP の扱い＝記録のみに決定（案B・2026-06-13・(3b)：絶対判定不可ゆえ合否・再聴トリガに使わず同一プロンプト内の相対比較のみ／再聴トリガは L3 が担う）**。残り＝**クラウド audio-LLM(L3) 選定（音声対応・モデル名/版）**／L1〜L3 不一致ルールの最終化。
