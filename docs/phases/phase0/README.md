...
### 5.3 判定の構造：hard gate は「互換・方向性」の2つだけ
...
**運用**：L1 を最終ラベルのアンカーにしつつ、**L0 は欠陥を自動NG**（耳に依らない）／**L2・L3 と不一致のクリップだけ聴き直す**（人間の見落とし・甘さの検出）。＝「開発者1人＝主観的」を、多数の独立判定＋客観DSPで構造的に緩和する。

**各判定者への OK/部分/NG の翻訳**（§5.2 の定義を、判定者ごとの形に）
...
**集計（同じ計測を3通りに数える）** — ラベル付けは1回。それを3単位で数える（→ §5.3 (b) の考え方）。

| 集計単位 | 定義 | 使い道 |
|---|---|---|
| クリップ単位＝**ヒット率** | 到達(OK+部分) ÷ 全クリップ | 1本ずつの当たり率 → **比較（選定）** |
| プロンプト単位＝**カバー率** | Best-of-N に到達が1本でもあるプロンプト ÷ 全プロンプト | 数回試せば届くか → **★hard gate** |
| モデル単位 | 上記＋速度・メモリ | gate＝カバー率／比較＝ヒット率ほか |

> 例：3プロンプト × Best-of-N=4本 ＝ 12クリップ。A[OK,OK,部分,NG]／B[部分,NG,NG,NG]／C[NG,NG,NG,NG] → **ヒット率=5/12≈42%**（比較用）、**カバー率=2/3≈67%**（gate用）。gate は甘いカバー率、優劣は厳しいヒット率。

**記録（ブラインド）** — 2つの表で「どのモデルが作ったか伏せて聴く」を実現する。
...
```mermaid
flowchart TD
    M["候補モデル（実測）"] --> G1{"互換性<br/>動くか（致命的エラー/fallback地獄でない）"}:::gate
    G1 -- NG --> X1["土俵外<br/>（Macで動かないなら主開発機で不可。CUDA動作は記録）"]:::x
    G1 -- OK --> G2{"品質（方向性）<br/>低い床：破綻・無音・無関係でない"}:::gate
...
- **合格ラインの具体値**（§5.3）：方向性OKの割合・速度の足切り値。**着手前に仮値→実測後に較正**（判定の構造と方法は §5.1〜§5.5 で確定済み・数値が未確定）。
