...
**この節は、§5.3 全体フロー図の2つ目の菱形「品質（方向性）ゲート」の中身**＝どう判定して、そのゲートの OK/NG を出すか、を詳述する（(1)→(4) でゲート判定を組み立て、結論が (4)）。
...
- **② 並列・独立判定**：欠陥なしのクリップを **L1(人間ブラインド)・L2(CLAP)・L3(LLM) が互いの結果を見ずに**判定（直列にすると L1 が他に引きずられる＝**アンカリングバイアス**で独立性が死ぬ）。※ここの **L1「人間（ブラインド）」を実装する仕組みが (5)**（どのモデルが作ったか伏せて聴く）。
...
**(4) 出口：クリップのラベルを集計して「品質（方向性）ゲート」の OK/NG を出す**（ラベル付けは1回、3単位で数える）

**ここが §5.3 全体フロー図の2つ目の菱形「品質（方向性）ゲート」の OK/NG を出す部分**（§5.4 全体がその菱形の中身、結論が本項）。クリップのラベル(2)を集計してモデル単位の判定にする：

- プロンプト＝**カバー**（Best-of-N に到達が1本でもあれば）
- モデル＝**カバー率 ≥ floor ならゲート＝OK（通過）／ floor 未満ならゲート＝NG（モデル変更）**

| 集計単位 | 定義 | 使い道 |
|---|---|---|
| クリップ単位＝**ヒット率** | 到達(OK+部分) ÷ 全クリップ | 1本ずつの当たり率 → **比較（選定）** |
| プロンプト単位＝**カバー率** | Best-of-N に到達が1本でもあるプロンプト ÷ 全プロンプト | 数回試せば届くか → **★ゲートの判定** |
| モデル単位 | 上記＋速度・メモリ | gate＝カバー率／比較＝ヒット率ほか |

> 例：3プロンプト × Best-of-N=4本 ＝ 12クリップ。A[OK,OK,部分,NG]／B[部分,NG,NG,NG]／C[NG,NG,NG,NG] → **ヒット率=5/12≈42%**（比較用）、**カバー率=2/3≈67%**（gate用）。仮に floor=50% なら 67%≥50% で **ゲート＝OK**。gate は甘いカバー率、優劣は厳しいヒット率。

**(5) 記録（ブラインド）** — **(2)② の「L1 人間（ブラインド）」を実装する仕組み**。2つの表で「どのモデルが作ったか伏せて聴く」を実現する。
