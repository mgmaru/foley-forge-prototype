...
| LLM（プロンプト変換） | 同上の抽象化レイヤー（ローカル軽量モデルを想定） | Step 3（プロンプト生成戦略の決定）※LLM採用時のみ | Step 2と同じ抽象化を共有し、ステップごとにプロバイダー／モデルを使い分け可能 |
...
| LLMプロバイダー抽象化 | Step 2/3のLLMを切替可能にし、クラウド品質とローカル完結（オフライン・コスト0）を両立。出力はスキーマ検証＋リトライで担保 |
