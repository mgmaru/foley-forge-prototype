...
| レイヤー | 技術 | 採用理由 |
|---------|------|---------|
| フロントエンド | React + Vite（ブラウザUI） | OS依存を抑え、ローカルFastAPIと組み合わせて動作。将来のクラウド版／ランチャー化にも展開しやすい |
| バックエンド | Python + FastAPI | 拡散モデル・CLAP等のエコシステムが充実。フロントとはHTTP/WebSocketで連携 |
| 生成エンジン（初期） | diffusers直叩き | 最小限の実装で生成アルゴリズム開発に集中 |
| 生成エンジン（将来） | ComfyUI連携 / 自前最適化 | GPU最適化・複数モデル管理 |
| 音声生成モデル | Stable Audio Open ほか複数対応 | アニメ寄り環境音に強い、複数モデル切替前提 |
| LLM（マルチモーダル） | Claude API（画像理解） | 画像理解の品質が高い |
| LLM（プロンプト変換） | Ollama（ローカル） | コスト0で使い倒せる |
