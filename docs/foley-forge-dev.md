...
- LLMプロバイダーの capability 差（画像理解・構造化出力）の吸収方法、および入力に応じたモデルルーティングの要否
- Step 2のJSON検証失敗時のリトライ戦略（上限回数、段階的リトライ、コードによる軽微修復の優先、打ち切り時の挙動）
- リファレンスとするLLMプロバイダー／T2Aモデルの確定
- `models.yaml` に持たせるモデル仕様の具体スキーマ（サンプルレート・最大長・条件付けの有無・戦略別パラメータ等）
- 評価ハーネスの仕様（固定お題集、使用する指標、集計・比較方法）
- モデルの品質ラベルをユーザに提示するか（開発者による静的ラベル／ユーザ自己診断の2モード、生スコアは非表示、「多様性の第4軸」との緊張関係を考慮）
